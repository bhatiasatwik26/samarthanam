...
export const TaskSummary: React.FC<TaskSummaryProps> = ({ events }) => {
  // Count events
  // INTEGRATION POINT: In the future, this events count can be fetched directly from the backend API
  // rather than calculating it from the events passed to this component.
  // Example API endpoint: GET /api/user/:userId/stats/eventsCount
  ...
  const completedTasks = events ? events.reduce((count, event) => {
    return count + (event.tasks ? event.tasks.filter(task => task.status === 'completed').length : 0);
  }, 0) : 0;
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-md p-6 mb-8 border border-gray-100 dark:border-gray-700">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="bg-gray-50 dark:bg-gray-700 rounded-lg p-4 border border-gray-100 dark:border-gray-600">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              <h4 className="text-sm font-medium text-gray-600 dark:text-gray-400">Events Participated</h4>
              <p className="text-2xl font-bold text-gray-800 dark:text-white">{eventsCount}</p>
            </div>
          </div>
        </div>
      ...
      <div className="mt-6">
        <div className="flex justify-between text-sm mb-1">
          ...
          <span className="font-medium text-gray-800 dark:text-white">{completionRate}%</span>
        </div>
